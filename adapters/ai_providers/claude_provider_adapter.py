# ===== adapters/ai_providers/claude_provider_adapter.py =====
"""
Claude Provider 어댑터 (기존 ClaudeProvider 리팩터링)
"""
...
class ClaudeProviderAdapter(BaseProviderAdapter):
    """Claude Provider 어댑터 (기존 ClaudeProvider 개선)"""

    # 기존 SUPPORTED_MODELS 유지
    SUPPORTED_MODELS = {
    ...
    def __init__(self, api_key: str = None, model_name: str = "claude-3-5-sonnet-20241022", **kwargs):
        ...
        try:
            ...
            raise ValueError(f"Claude 클라이언트 초기화 실패: {e}")
    ...
    def chat(self, messages: List[ChatMessage], **kwargs) -> ChatResponse:
        """Claude와 채팅 (기존 로직 + 성능 추적)"""
        start_time = time.time()
        success = False

        try:
            if not self.is_available():
                raise RuntimeError("Claude API가 사용 불가능합니다")
            ...
            # 토큰 사용량 추정 (Claude API는 정확한 토큰 수 제공하지 않음)
            ...
            # 성능 메트릭 업데이트
            self._update_performance_metrics(response_time, success, cost_estimate)
            ...
            return ChatResponse(
                ...
                metadata={"provider": "claude", "raw_response": response}
            )
    ...
    def get_capabilities(self) -> List[str]:
        """지원하는 기능 목록"""
        return ["chat", "streaming", "cost_estimation", "high_quality_reasoning"]
